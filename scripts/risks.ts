...
function getLevelArgs(): string[] {
  ...
  if (reportLevelArgs.some((it) => !valid.includes(it))) {
    exitWithMessage(
      `Invalid --levels, given: ${reportLevelArgs}, valid: ${valid}`,
      1,
    );
  }
...
async function getVulnerabilities(team: string, levels: string[]) {
  ...
  return R.pipe(
    ...
    R.filter((it) =>
      levels.includes(it.vulnerability.securityVulnerability.severity),
    ),
...
function vulnSlackLine({
  ...
  return `${pkg.ecosystem}: ${
    pkg.name
  }@${vulnerableVersionRange}, ${formatDistanceToNowStrict(
    parseISO(createdAt),
    {
      unit: "day",
      locale: nb,
    ...
    firstPatchedVersion?.identifier
      ? `Fixed in *${firstPatchedVersion?.identifier}*`
      : "NO FIX"
  } <${url}/security/dependabot/${number}|issue>, <${advisory.permalink}|CVE>`;
}

const levels: string[] = getLevelArgs();
const vulnerabilitiesByLevel = await getVulnerabilities(
  "teamsykmelding",
  levels,
);
  ...
  await postBlocks([
    {
      ...
      text: {
        type: "mrkdwn",
        text: `:cool_pepe: Ingen sårbarheter funnet på nivåene i dag ${levels.join(
          ", ",
        )} :cool_pepe:`,
      },
    },
